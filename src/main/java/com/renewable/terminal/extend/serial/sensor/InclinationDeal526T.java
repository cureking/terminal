 ...
 */
@Slf4j
public class InclinationDeal526T {
	...
	public static ServerResponse<byte[]> command2Origin(int address, InclinationConst.InclinationSensor1Enum inclinationSensor1Enum, Integer data) {
		byte[] originData;
		...
		if (command == InclinationConst.InclinationSensor1Enum.SETZERO.getCode()) {
			//生成指令数据帧   todo 还没修改
			originData = list2OriginData(objece2ListData(command, address, data));
		} else if (command == InclinationConst.InclinationSensor1Enum.GETZERO.getCode()) {
	...
	private static Inclination originData2Object(byte[] originData) {
		...
		double origin_X = origin2Double(OtherUtil.subBytes(originData, 4, 3));
		double origin_Y = origin2Double(OtherUtil.subBytes(originData, 7, 3));
...
//        System.out.println("origin_X:" + origin_X);
		double origin_T = origin2Double(OtherUtil.subBytes(originData, 10, 3));
...
//        System.out.println(Arrays.toString(serverResponse.getData()));

//        System.out.println(Arrays.toString(objece2ListData(04,00).toArray()));
//        System.out.println(Arrays.toString(list2OriginData(objece2ListData(04, 00))));
...
////        origin2Object(test3);
		...
		//TODO 2019.04.12 完成了数据格式的大部分任务，之后可以将数据库的数据表 的数据改为4.4的。另外需要清洗掉（0，0，0）的情况，但又要避免真正的0。（话说，三个参数，包括温度都是0，基本可以默认是残废数据了）。
